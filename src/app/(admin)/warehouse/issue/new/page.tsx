...
function StockIssueForm() {
  ...
  const [fromWarehouse, setFromWarehouse] = useState(initialWarehouse);
  const [postingDate, setPostingDate] = useState(new Date().toISOString().split("T")[0]);
  const [remarks, setRemarks] = useState("Daily Kitchen Food Preparation & Cooking Consumption");
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        if (!initialWarehouse) setFromWarehouse("Main Stores - CP");
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fromWarehouse) {
      setErrorMsg("Source Warehouse is required.");
      return;
    }

    const validItems = items.filter((i) => i.item_code && i.qty > 0);
    if (validItems.length === 0) {
      setErrorMsg("Please specify at least one valid item to issue.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMsg("Stock Issue voucher created successfully!");
        setTimeout(() => {
          router.push(`/warehouse/${encodeURIComponent(fromWarehouse)}`);
        }, 800);
      } else {
        const json = await res.json().catch(() => ({}));
        setErrorMsg(json.message || "Failed to submit Stock Issue to Frappe.");
      }
    } catch (err: any) {
    ...
    <form onSubmit={handleSubmit} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
      ...
      <div className="grid sm:grid-cols-2 gap-3.5">
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-slate-700">
            Source Warehouse (Deduct From) <span className="text-rose-500">*</span>
          </label>
        ...
        <div className="space-y-1 sm:col-span-2">
          <label className="text-xs font-bold text-slate-700">
            Issue Purpose / Recipe Notes
          </label>
          <Input
            type="text"
            value={remarks}
            onChange={(e) => setRemarks(e.target.value)}
            placeholder="e.g. Daily Lunch Preparation for Nairobi HQ Canteen"
      ...
      <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2.5">
        ...
        <Button
          type="submit"
          disabled={submitting}
          className="h-9 px-5 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs rounded-xl shadow-md shadow-rose-600/20 cursor-pointer"
        >
          <Save className="size-3.5 mr-1" />
          <span>{submitting ? "Posting to ERP…" : "Submit Stock Issue"}</span>
        </Button>
      </div>
    </form>
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 px-4 rounded-xl border border-slate-200 shadow-sm">
        <nav className="flex items-center gap-1.5 text-xs text-slate-500 flex-wrap">
          ...
          <span className="font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded-md">
            New Stock Issue (Kitchen Consumption)
          </span>
        </nav>
        ...
        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-rose-50 border border-rose-200 text-rose-800 text-[10px] font-bold">
          <ArrowUpFromLine className="size-3 text-rose-600" /> Material Issue
        </span>
      </div>

      <Suspense fallback={<div className="p-8 text-center text-xs font-bold text-slate-500">Loading stock issue form…</div>}>
        <StockIssueForm />
      </Suspense>
    </div>
  );
